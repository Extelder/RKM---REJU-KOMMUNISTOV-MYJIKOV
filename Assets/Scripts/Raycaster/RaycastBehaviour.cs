...
public class RaycastBehaviour : MonoBehaviour
{
    ...
    [field: SerializeField] public bool DrawGizmos { get; private set; }

    private void OnDrawGizmosSelected()
    {
        if (DrawGizmos)
            Gizmos.DrawRay(Camera.transform.position, Camera.forward * Range);
    }
    ...
    public bool GetHitCollider(out Collider collider, out RaycastHit Hit)
    {
        Hit = GetRaycastHit();
